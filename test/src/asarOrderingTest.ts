...
// End to end: `asar.ordering` really controls the pack order, and it composes with `asar.contentAlignment`
// (ordering decides the sequence, alignment only pads between files) — which is what lets an app drop an
// afterPack re-pack that would otherwise undo alignment.
...
test.ifNotWindows("asar.ordering packs listed files first and composes with contentAlignment", ({ expect }) =>
  app(
    expect,
    {
      targets: linuxDirTarget,
      config: {
        asar: { ordering: "asar-order.txt", contentAlignment: 512 },
      },
    },
    {
      ...
      packed: async context => {
        ...
        for (const [rel, offset] of offsets) {
          expect(offset % 512, `${rel} @ ${offset}`).toBe(0)
        }
      },
    }
  )
